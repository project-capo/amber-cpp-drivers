...
RoboclawController::RoboclawController(int pipeInFd, int pipeOutFd,
...
>>>>>>> branch 'szszRoboClawFix' of https://github.com/project-capo/amber-cpp-drivers.git
	...
	if (_configuration->error_monitor_interval > 0) {
		_errorMonitorThread = new boost::thread(
				boost::bind(&RoboclawController::errorMonitor, this));
	}
...
void RoboclawController::handleDataMsg(amber::DriverHdr *driverHdr,
	...
	// TODO: hack for now
	int clientId =
			driverHdr->clientids_size() > 0 ? driverHdr->clientids(0) : 0;
	...
	if (driverMsg->HasExtension(roboclaw_proto::currentSpeedRequest)) {
		...
		if (!driverMsg->has_synnum()) {
			LOG4CXX_WARN(_logger,
					"Got CurrentSpeedRequest, but syn num not set. Ignoring.");
			return;
		}
		...
		handleMotorsEncoderCommand(
				driverMsg->MutableExtension(roboclaw_proto::motorsCommand));
	}
...
amber::DriverMsg *RoboclawController::buildCurrentSpeedMsg() {
	...
	roboclaw_proto::MotorsSpeed *currentSpeed = message->MutableExtension(
			roboclaw_proto::currentSpeed);

	MotorsSpeedStruct mc;
	bool speedReadSuccess = true;

	mc = readRoboClawSpeed;
...
void RoboclawController::handleMotorsEncoderCommand(
		roboclaw_proto::MotorsSpeed *motorsCommand) {
	if (_batteryLow) {
		return;
	}
...
int RoboclawController::toMmps(int in) {
	int out = (int) (in * (int) _configuration->wheel_radius * M_PI * 2
			/ (double) _configuration->pulses_per_revolution);
...
void RoboclawController::batteryMonitor() {
	LOG4CXX_INFO(_logger,
			"Battery monitor thread started, interval: " << _configuration->battery_monitor_interval << "ms");
	...
	while (1) {
		boost::this_thread::sleep(
				boost::posix_time::milliseconds(
						_configuration->battery_monitor_interval));
		...
		if (!_roboclawDisabled) {
			...
			try {
				...
				LOG4CXX_INFO(_logger,
						"Main battery voltage level: " << voltage/10.0 << "V");
...
void RoboclawController::errorMonitor() {
	LOG4CXX_INFO(_logger,
			"Hardware error monitor started, interval: " << _configuration->error_monitor_interval << "ms");

	__u8 frontErrorStatus, rearErrorStatus, frontErrorStatusTmp,
			rearErrorStatusTmp;
	bool same_errors;
	...
	while (1) {
		boost::this_thread::sleep(
				boost::posix_time::milliseconds(
						_configuration->error_monitor_interval));

		try {
			_roboclawDriver->readErrorStatus(&frontErrorStatus,
					&rearErrorStatus);

			if (frontErrorStatus != RC_ERROR_NORMAL
					|| rearErrorStatus != RC_ERROR_NORMAL) {
				...
				for (unsigned int i = 0;
						i < _configuration->critical_read_repeats; i++) {
					_roboclawDriver->readErrorStatus(&frontErrorStatus,
							&rearErrorStatus);

					if (frontErrorStatus != frontErrorStatusTmp
							|| rearErrorStatus != rearErrorStatusTmp) {
						same_errors = false;
						break;
					}
				}
				...
				if (same_errors) {
					if (frontErrorStatus != RC_ERROR_NORMAL) {
						LOG4CXX_WARN(_logger,
								"Front Roboclaw error: " << getErorDescription(frontErrorStatus));
					}

					if (rearErrorStatus != RC_ERROR_NORMAL) {
						LOG4CXX_WARN(_logger,
								"Rear Roboclaw error: " << getErorDescription(rearErrorStatus));
					}

					if (frontErrorStatus == RC_ERROR_M1_OVERCURRENT
							|| frontErrorStatus == RC_ERROR_M2_OVERCURRENT
							|| rearErrorStatus == RC_ERROR_M1_OVERCURRENT
							|| rearErrorStatus == RC_ERROR_M2_OVERCURRENT) {

						resetAndWait();
					} else if (frontErrorStatus == RC_ERROR_MAIN_BATTERY_LOW
							|| rearErrorStatus == RC_ERROR_MAIN_BATTERY_LOW) {
...
void RoboclawController::temperatureMonitor() {
	LOG4CXX_INFO(_logger,
			"Temperature monitor thread started, interval: " << _configuration->temperature_monitor_interval << "ms");
	...
	while (1) {
		boost::this_thread::sleep(
				boost::posix_time::milliseconds(
						_configuration->temperature_monitor_interval));
		...
		if (!_roboclawDisabled) {
			_roboclawDriver->readTemperature(&frontTemperature,
					&rearTemperature);

			LOG4CXX_INFO(_logger,
					"Front temperature: " << frontTemperature/10.0 << "C, " << "rear temperature: " << rearTemperature/10.0 << "C");

			if (_overheated) {
				// if temperature droped down below drop level
				if (frontTemperature < _configuration->temperature_drop
						&& rearTemperature < _configuration->temperature_drop) {
					_overheated = false;
					...
					for (unsigned int i = 0;
						...
						if (frontTemperature > _configuration->temperature_drop
								|| rearTemperature
										> _configuration->temperature_drop) {
							_overheated = true;
							break;
						}
					}
				...
				if (frontTemperature > _configuration->temperature_critical
						|| rearTemperature
								> _configuration->temperature_critical) {
					...
					if (_overheated) {
						...
						LOG4CXX_WARN(_logger,
								"Roboclaw _overheated, waiting for cool down to " << _configuration->temperature_drop/10.0 << "C");
					}
				}
			}
...
void RoboclawController::resetAndWait() {
	...
	LOG4CXX_INFO(_logger,
			"Reseting Roboclaws and waiting " << _configuration->reset_delay << "ms");

	_roboclawDisabled = true;

	_roboclawDriver->reset();
	boost::this_thread::sleep(
			boost::posix_time::milliseconds(_configuration->reset_delay));
...
void RoboclawController::parseConfigurationFile(const char *filename) {
	...
	desc.add_options()("roboclaw.uart_port",
			value<string>(&_configuration->uart_port)->default_value(
					"/dev/ttyO3"))("roboclaw.uart_speed",
			value<unsigned int>(&_configuration->uart_speed)->default_value(
					38400))("roboclaw.reset_gpio_path",
			value<string>(&_configuration->reset_gpio_path)->default_value(
					"/sys/class/gpio/gpio136/value"))("roboclaw.reset_delay",
			value<unsigned int>(&_configuration->reset_delay)->default_value(
					260))("roboclaw.led1_gpio_path",
			value<string>(&_configuration->led1_gpio_path)->default_value(
					"/sys/class/gpio/gpio139/value"))("roboclaw.led2_gpio_path",
			value<string>(&_configuration->led2_gpio_path)->default_value(
					"/sys/class/gpio/gpio137/value"))(
			"roboclaw.front_rc_address",
			value<unsigned int>(&front_rc_address)->default_value(128))(
			"roboclaw.rear_rc_address",
			value<unsigned int>(&rear_rc_address)->default_value(129))(
			"roboclaw.motors_max_qpps",
			value<unsigned int>(&_configuration->motors_max_qpps)->default_value(
					13800))("roboclaw.motors_p_const",
			value<unsigned int>(&_configuration->motors_p_const)->default_value(
					65536))("roboclaw.motors_i_const",
			value<unsigned int>(&_configuration->motors_i_const)->default_value(
					32768))("roboclaw.motors_d_const",
			value<unsigned int>(&_configuration->motors_d_const)->default_value(
					16384))("roboclaw.pulses_per_revolution",
			value<unsigned int>(&_configuration->pulses_per_revolution)->default_value(
					1865))("roboclaw.wheel_radius",
			value<unsigned int>(&_configuration->wheel_radius)->default_value(
					60))("roboclaw.battery_monitor_interval",
			value<unsigned int>(&_configuration->battery_monitor_interval)->default_value(
					0))("roboclaw.error_monitor_interval",
			value<unsigned int>(&_configuration->error_monitor_interval)->default_value(
					0))("roboclaw.temperature_monitor_interval",
			value<unsigned int>(&_configuration->temperature_monitor_interval)->default_value(
					0))("roboclaw.temperature_critical",
			value<__u16 >(&_configuration->temperature_critical)->default_value(
					70))("roboclaw.temperature_drop",
			value<__u16 >(&_configuration->temperature_drop)->default_value(60))(
			"roboclaw.critical_read_repeats",
			value<unsigned int>(&_configuration->critical_read_repeats)->default_value(
					0))("roboclaw.stop_idle_timeout",
			value<unsigned int>(&_configuration->stop_idle_timeout)->default_value(
					1000))("roboclaw.reset_idle_timeout",
			value<unsigned int>(&_configuration->reset_idle_timeout)->default_value(
					10000));

	_configuration->commend_monitor_interval = 100;
	...
	try {
		...
		LOG4CXX_ERROR(_logger,
				"Error in parsing configuration file: " << e.what());
		exit(1);
	}
...
void RoboclawController::commendMonitor()
{
	while (1)
	{
		...
		boost::this_thread::sleep(boost::posix_time::milliseconds(_configuration->commend_monitor_interval));
	...
	while (1)
	{
...

		{
			...
			if (_resetTime <= actTime) {
				...
				_resetTime = actTime
						+ boost::posix_time::milliseconds(
								_configuration->reset_idle_timeout);
			}
		}
...
void RoboclawController::resetTimeouts() {
	...
	_resetTime = actTime
			+ boost::posix_time::milliseconds(
					_configuration->reset_idle_timeout);
	_motorsStopTime = actTime
			+ boost::posix_time::milliseconds(
					_configuration->stop_idle_timeout);
...
int main(int argc, char *argv[]) {
	...
	LOG4CXX_INFO(logger, "-------------");
	LOG4CXX_INFO(logger,
			"Creating controller, config_file: " << argv[1] << ", log_config_file: " << argv[2]);
